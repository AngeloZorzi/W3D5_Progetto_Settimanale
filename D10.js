...
*/
const deleteOne = function (s, booleano) {
  if (booleano === true) {
    return s.slice(1);
  } else {
    return s.slice(0, -1);
...
const onlyLetters = function (s) {
  const removeNumbers = s.replace(/\d/g, "");
  return removeNumbers;
};
console.log(onlyLetters("a123iu473to09"));
/* ESERCIZIO 6
...
console.log(isThisAnEmail("angelo@xgoo.com"));
console.log(isThisAnEmail("angeloxgoo.com"));
/* ESERCIZIO 7
...
const howManyDays = function (date1) {
  ...
  const daysgone = (date2.getTime() - date1.getTime()) / (1000 * 3600 * 24);
  return Math.floor(daysgone);
};
console.log(howManyDays(new Date("2024-02-21")));
/* ESERCIZIO 10
  Scrivi una funzione chiamata "isTodayMyBirthday" che deve ritornare true se oggi è il tuo compleanno, falso negli altri casi.
*/
const isTodayMyBirthday = function () {
  const today = new Date();
  const birthday = new Date(today.getFullYear(), 2, 20);
  if (
...
const onlyTheYears = function () {
  ...
  for (let i = 0; i < movies.length; i++) {
    newArrayYear.push(movies[i].Year);
  }
  return newArrayYear;
};
